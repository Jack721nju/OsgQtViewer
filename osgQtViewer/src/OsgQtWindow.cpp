...
#define Icon_Path QString::fromStdString(JsonMgr::getReadValue()["IconPath"].asString())
#define Data_Path QString::fromStdString(JsonMgr::getReadValue()["DataPath"].asString())
...
void OsgQtTest::slot_OpenData() {
	...
	const std::string &fullname = getFullName.toStdString();//全局名称，包含完整路径和文件名
	const std::string &path_name = curFilePath.toStdString();//完整路径

	const std::string &type_name = curFileSuffix.toStdString();//文件类型
	const std::string &file_name = curFileName.toStdString();//文件名称
...
void OsgQtTest::ReadTxtData(const std::string & fileName) {
	...
	line_count = static_cast<size_t>((text_file.size() << 8) / single_line_bytes);//约为点数量
